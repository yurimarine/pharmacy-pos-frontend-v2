...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------- |
| `ADMIN_ONLY_ROUTES` (users, suppliers, manufacturers, pharmacies, discounts, product-classes, product-categories, packaging-units, dispensing-units, till-sessions, time-logs, warehouse, warehouse-receipts, stock-transfers, inventory-logs) | `admin` only                       |
| `ADMIN_PHARMACIST_ROUTES` (dashboard, inventory, batches, transactions, products, reports, purchase-orders)                                                                    | `admin`, `pharmacist`              |
...
- `NavMain` — Dashboard, Inventory, Products, Batches, Purchase Orders, Warehouse Receipts (admin), Warehouse (admin), Stock Transfers (admin), Transactions, POS Terminal (pharmacist only, external)
...
- On save, each non-empty creatble field is upserted via `upsertProductSuggestions()` (fire-and-forget) so it appears in future dropdowns.
