...
export function BatchDetail({ batch: initialBatch, isAdmin }: BatchDetailProps) {
        ...
        <CardHeader>
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              <CardTitle className="font-mono text-xl">{batch.batch_number}</CardTitle>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
            <div className="flex flex-col gap-1">
              ...
              <span className="text-sm font-medium">{batch.pharmacies?.name ?? "—"}</span>
            </div>
            <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-1">
              ...
              <span className="text-sm font-medium">{formatDate(batch.created_at)}</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-xs text-muted-foreground">Last Updated</span>
              <span className="text-sm font-medium">{formatDate(batch.updated_at)}</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-xs text-muted-foreground">Items</span>
              <span className="text-sm font-medium">{items.length}</span>
            </div>
            {batch.type === "stock_in" && batch.transfer_to_pharmacy_id == null ? null : null}
          </div>
          {batch.notes && (
          ...
          <Table>
            ...
            <TableBody>
              {items.length === 0 ? (
                <TableRow>
                  <TableCell
                    colSpan={batch.type === "stock_in" ? (isDraft ? 9 : 8) : (isDraft ? 7 : 6)}
                    className="text-center text-muted-foreground py-8"
                  >
                    No items added yet.{isDraft && ' Click "Add Item" to get started.'}
                  </TableCell>
                </TableRow>
              ) : (
                items.map((item) => (
