...
export function AddBatchItemModal({
  ...
  const selectedProductChange = products.find(
    p => p.id === selectedProductIdChange,
  );
        ...
        <div className="flex-1 overflow-y-auto px-1">
          {isPriceChange ? (
            <form
              id="add-item-form"
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              onSubmit={priceChangeForm.handleSubmit(
                handleSubmitPriceChange as any,
              )}
            ...
            >
              ...
              {selectedProductChange && (
                <p className="text-sm text-muted-foreground -mt-2">
                  Current base price: ₱
                  {Number(selectedProductChange.base_price).toFixed(2)}
                </p>
              )}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {selectedProductOut?.current_quantity !== undefined &&
                  quantityOut > selectedProductOut.current_quantity && (
                    <p className="text-sm text-destructive">
                      Only {selectedProductOut.current_quantity} units
                      available.
                    </p>
                  )}
              </div>
