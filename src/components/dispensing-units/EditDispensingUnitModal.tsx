...
import { updateDispensingUnit } from "@/app/admin/dispensing-units/actions"
...
export function EditDispensingUnitModal({ open, onOpenChange, unit }: Props) {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    startTransition(async () => {
      try {
        await updateDispensingUnit(unit.id, data)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Edit Dispensing Unit</DialogTitle>
          <DialogDescription>
            Update the details of this dispensing unit.
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="edit-disp-name">
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="edit-disp-abbreviation">
              Abbreviation <span className="text-destructive">*</span>
            </Label>
            <Input
              id="edit-disp-abbreviation"
              placeholder="e.g. tab"
              maxLength={6}
