...
export function composeProductName(data: {
  ...
  let suffix = ''
  if (volume) {
    suffix = ` ${packagingUpper} ${volume.toUpperCase()}`
  } else if (unitCount > 1) {
...
export function computeSellingPrice(unitCost: number, markupPercentage: number): number {
  return Math.round(unitCost * (1 + markupPercentage / 100) * 100) / 100
}

export function computeMarkupPercentage(unitCost: number, sellingPrice: number): number {
